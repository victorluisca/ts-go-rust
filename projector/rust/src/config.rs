use std::path::PathBuf;

use anyhow::{Context, Ok, Result, anyhow};

use crate::opts::ProjectorOptions;

#[derive(Debug)]
pub struct Config {
    pub operation: Operation,
    pub pwd: PathBuf,
    pub config: PathBuf,
}

impl TryFrom<ProjectorOptions> for Config {
    type Error = anyhow::Error;

    fn try_from(value: ProjectorOptions) -> Result<Self> {
        let operation = value.arguments.try_into()?;
        let config = get_config(value.config)?;
        let pwd = get_pwd(value.pwd)?;

        return Ok(Config {
            operation,
            config,
            pwd,
        });
    }
}

#[derive(Debug, PartialEq)]
pub enum Operation {
    Print(Option<String>),
    Add(String, String),
    Delete(String),
}

impl TryFrom<Vec<String>> for Operation {
    type Error = anyhow::Error;

    fn try_from(value: Vec<String>) -> Result<Self> {
        let mut value = value;

        if value.len() == 0 {
            return Ok(Operation::Print(None));
        }

        let term = value.get(0).expect("to exist");
        if term == "add" {
            if value.len() != 3 {
                return Err(anyhow!(
                    "operation add expects 2 arguments but got {}",
                    value.len() - 1
                ));
            }

            let mut drain = value.drain(1..=2);

            return Ok(Operation::Add(
                drain.next().expect("to exist"),
                drain.next().expect("to exist"),
            ));
        }

        if term == "del" {
            if value.len() != 2 {
                return Err(anyhow!(
                    "operation delete expects 1 argument but got {}",
                    value.len() - 1
                ));
            }

            let arg = value.pop().expect("to exist");
            return Ok(Operation::Delete(arg));
        }

        if value.len() > 1 {
            return Err(anyhow!(
                "operation print expects 0 or 1 arguments but got {}",
                value.len() - 1
            ));
        }

        let arg = value.pop().expect("to exist");
        return Ok(Operation::Print(Some(arg)));
    }
}

fn get_config(config: Option<PathBuf>) -> Result<PathBuf> {
    if let Some(value) = config {
        return Ok(value);
    }

    let location = std::env::var("XDG_CONFIG_HOME").context("unable to get XDG_CONFIG_HOME")?;
    let mut location = PathBuf::from(location);

    location.push("projector");
    location.push("projector.json");

    return Ok(location);
}

fn get_pwd(pwd: Option<PathBuf>) -> Result<PathBuf> {
    if let Some(pwd) = pwd {
        return Ok(pwd);
    }

    return Ok(std::env::current_dir().context("error getting current directory")?);
}

#[cfg(test)]
mod test {
    use anyhow::Result;

    use crate::{config::Operation, opts::ProjectorOptions};

    use super::Config;

    #[test]
    fn test_print_all() -> Result<()> {
        let opts: Config = ProjectorOptions {
            arguments: vec![],
            pwd: None,
            config: None,
        }
        .try_into()?;

        assert_eq!(opts.operation, Operation::Print(None));

        return Ok(());
    }

    #[test]
    fn test_print_key() -> Result<()> {
        let opts: Config = ProjectorOptions {
            arguments: vec![String::from("foo")],
            pwd: None,
            config: None,
        }
        .try_into()?;

        assert_eq!(opts.operation, Operation::Print(Some(String::from("foo"))));

        return Ok(());
    }

    #[test]
    fn test_add_key_value() -> Result<()> {
        let opts: Config = ProjectorOptions {
            arguments: vec![
                String::from("add"),
                String::from("foo"),
                String::from("bar"),
            ],
            pwd: None,
            config: None,
        }
        .try_into()?;

        assert_eq!(
            opts.operation,
            Operation::Add(String::from("foo"), String::from("bar"))
        );

        return Ok(());
    }

    #[test]
    fn test_delete_key() -> Result<()> {
        let opts: Config = ProjectorOptions {
            arguments: vec![String::from("del"), String::from("foo")],
            pwd: None,
            config: None,
        }
        .try_into()?;

        assert_eq!(opts.operation, Operation::Delete(String::from("foo")));

        return Ok(());
    }
}
